...
class NegoDecisionLogic(BaseDecisionLogic):
    def get_decision(self,perceptions):
        ...
        """
        decs=[a.decisions(p) for a,p in zip(self.model.schedule.agents,perceptions)] # call each agent's decision fct with the appropriate perception
        self.last_actions=[{"production":p["production"],"cost":a["cost"],"consumption":a["consumption"],"agentID":a.unique_id,"action":d,"new_production":a.current_state["perception"]["production"],"new_consumption":a.current_state["perception"]["consumption"]} for a,p,d in zip(self.model.schedule.agents,perceptions,decs)]
